...
class EventBusV6:
    ...
    def on(
        ...
        *,
        ...
        Args:
            ...
            handler: 处理器函数
            priority: 优先级（数值越小越先执行，参见 EventPriority）
            once: 是否只执行一次
    ...
    def _register_handler(
        ...
        # 创建 HandlerInfo（确保优先级为整数或 IntEnum）
        info = HandlerInfo(
        ...
        with self._lock:
            ...
            # 按优先级排序（数值越小优先级越高）
            self._handlers[event_type].sort(key=lambda h: int(h.priority))
    ...
    def emit(self, event: Any, **kwargs) -> EmitResult:
        ...
        if not handlers:
            ...
            if self._dead_letter_queue:
                # 对部分事件类型（如 registry.*）无订阅是正常行为，不视为死信
                if not event_type.startswith("registry."):
    ...
    def _get_handlers(self, event_type: str) -> List[HandlerInfo]:
        ...
        # 排序（数值越小优先级越高）
        handlers.sort(key=lambda h: int(h.priority))
        return handlers
